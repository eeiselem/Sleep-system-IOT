...
// Server config (https://… for Cloudflare tunnel; http://… for LAN). Path /biometric is appended in post_data.
const String server_url = "YOUR_SERVER_IP";
// Must match Flask .env INGEST_API_KEY (same as main ESP32 /post-data).
...
// Posts AES-128-CBC ciphertext to POST /biometric (same INGEST_API_KEY as /post-data).
void post_data() {
  if (lastValidHeartRate > 0 && lastValidSpo2 > 0) {
    Serial.println("Attempting to send data...");

    String full_server_url = server_url + "/biometric";
    ...
    String payload;
    serializeJson(data_doc, payload);
    ...
    // WiFiClientSecure must stay alive until http.POST + http.end (do not declare inside `if`).
    HTTPClient http;
    ...
    int http_response_code = http.POST(encryptedPayload);
